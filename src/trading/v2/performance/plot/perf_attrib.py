...
def perf_attrib(
    ...
    Preprocesses inputs, and then calls perf_attrib. See
    perf_attrib for more info.
...
def compute_exposures(
    ...
    Normalizes positions (if necessary) and calls compute_exposures.
    ...
    return compute_exposures(positions, factor_loadings)
...
def create_perf_attrib_stats(perf_attrib, risk_exposures):
    ...
    annualized_returns_by_factor = [
        annual_return(perf_attrib[c]) for c in risk_exposures.columns
    ]
    cumulative_returns_by_factor = [
        cum_returns_final(perf_attrib[c]) for c in risk_exposures.columns
    ]
...
def plot_returns(perf_attrib_data, cost=None, ax=None):
    ...
    ax.plot(
        cum_returns(specific_returns), color="g", label="Cumulative specific returns"
    )
    ax.plot(cum_returns(common_returns), color="r", label="Cumulative common returns")

    if cost is not None:
        ax.plot(-cum_returns(cost), color="k", label="Cumulative cost spent")
...
def plot_factor_contribution_to_perf(
    ...
    for factor in factors_to_plot:
        factors_cumulative[factor] = cum_returns(factors_to_plot[factor])
