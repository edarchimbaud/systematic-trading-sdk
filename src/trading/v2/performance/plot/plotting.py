...
def plot_annual_returns(returns, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    ann_ret_df = pd.DataFrame(aggregate_returns(returns, "yearly"))
...
def show_perf_stats(
    ...
    if live_start_date is not None:
        live_start_date = metrics_utils.get_utc_timestamp(live_start_date)
...
def plot_rolling_returns(
    ...
    **kwargs, optional
    ...
    if factor_returns is not None:
        cum_factor_returns = cum_returns(factor_returns[cum_rets.index], 1.0)
...
def plot_exposures(returns, positions, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    ax.plot(
        net_exp.index, net_exp.values, label="Net", color="black", linestyle="dotted"
    )
    ...
    **kwargs, optional
    ...
    if live_start_date is not None:
        ...
        red_dots = matplotlib.lines.Line2D(
            [], [], color="red", marker="d", label="Out-of-sample data", linestyle=""
        )
...
def plot_slippage_sweep(
    ...
    **kwargs, optional
    ...
    for bps in slippage_params:
        ...
        label = str(bps) + " bps"
        slippage_sweep[label] = cum_returns(adj_returns, 1)
...
def plot_slippage_sensitivity(returns, positions, transactions, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    for bps in range(1, 100):
        ...
        avg_returns = annual_return(adj_returns)
...
def plot_daily_volume(returns, transactions, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    ax.axhline(
        daily_txn.txn_shares.mean(), color="steelblue", linestyle="--", lw=3, alpha=1.0
    )
...
def plot_cones(
    ...
    returns = cum_returns(oos_returns, starting_value=1.0)
