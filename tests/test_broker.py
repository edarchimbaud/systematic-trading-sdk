...
def test_buy_future():
    ...
    broker.buy_future(contract=Contract(ric=ric), contract_number=1)
...
def test_sell_future():
    ...
    broker = Broker(cash=1e6, live=False)
    broker.day = date(2022, 3, 7)
    broker.sell_future(contract=Contract(ric=ric), contract_number=1)
...
def test_close_future():
    ...
    broker.day = date(2022, 3, 7)
    broker.buy_future(contract=Contract(ric=ric), contract_number=1)
    broker.close_future(contract=Contract(ric=ric))
...
def test_expire_future():
    ...
    broker.day = date(2022, 3, 7)
    broker.buy_future(contract=Contract(ric=ric), contract_number=1)
    broker.expire_future(contract=Contract(ric=ric))
    assert broker.positions["Cash"]["USD"] == 999936.4757220994
    assert broker.positions[FUTURE_TYPE][ric] == 0
