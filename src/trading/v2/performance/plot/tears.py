...
from . import txn
from . import utils
from ..metrics import cum_returns, utils as metrics_utils
...
def create_returns_tear_sheet(
    ...
    if benchmark_rets is not None:
        returns = metrics_utils.clip_returns_to_benchmark(returns, benchmark_rets)
...
def create_txn_tear_sheet(
    ...
    plotting.plot_turnover(
        returns, transactions, positions, turnover_denom=turnover_denom, ax=ax_turnover
    )
    ...
    try:
        plotting.plot_daily_turnover_hist(
            transactions, positions, turnover_denom=turnover_denom, ax=ax_turnover_hist
        )
    except ValueError:
...
def create_interesting_times_tear_sheet(
    ...
    if not rets_interesting:
        warnings.warn(
            "Passed returns do not overlap with any" "interesting times.", UserWarning
        )
        return
    ...
    for i, (name, rets_period) in enumerate(rets_interesting.items()):
        ...
        cum_returns(rets_period).plot(
            ax=ax, color="forestgreen", label="algo", alpha=0.7, lw=2
        )

        if benchmark_rets is not None:
            cum_returns(bmark_interesting[name]).plot(
                ax=ax, color="gray", label="benchmark", alpha=0.6
            )
            ax.legend(
                ["Algo", "benchmark"], loc=legend_loc, frameon=True, framealpha=0.5
            )
        else:
