...
def omega_ratio(
    returns, risk_free=0.0, required_return=0.0, annualization=APPROX_BDAYS_PER_YEAR
):
...
def alpha_beta(
    returns, factor_returns, risk_free=0.0, period=DAILY, annualization=None, out=None
):
...
def alpha_aligned(
    ...
    out = np.subtract(
        np.power(
            np.add(np.nanmean(alpha_series, axis=0, out=out), 1, out=out),
            ann_factor,
            out=out,
        ),
        1,
